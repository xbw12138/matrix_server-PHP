...
if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
   echo "The file ".  basename( $_FILES['uploadedfile']['name']). " has been uploaded";
	...
	$str=basename( $_FILES['uploadedfile']['name']);//获取文件名
  	$response["status"] = 1;
	$url="http://115.159.26.120/androidapp_matrix/image_head_upload/upload/{$str}";//获取文件url路径
	...
	$sql="update user_information set user_image_head='$url' where user_phone='$phone[0]'";//手机号查找图片
