...
if (isset($_POST['user_phone'])&&isset($_POST['page'])) {
	...
	$num=$rowz[0]-$page*10;
	if($num>=0){
		$sql="select time,danmu_content,user_phone,id,talk_zan,phonety from danmu  limit $num , 10";
		...
		$jj=$num+10;
		$sql="select time,danmu_content,user_phone,id,talk_zan,phonety from danmu limit 0, $jj";
		if($result=mysql_query($sql)){
			while($row=mysql_fetch_array($result)){
				$sqls="select user_name,user_image_head from user_information where user_phone='$row[2]'";
				$rowzzz=mysql_fetch_row(mysql_query("select count(*) from pl_information where pl_num='$row[3]'"));
