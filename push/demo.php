...
if (isset($_GET['id'])&&isset($_GET['token'])){
	$id = $_GET['id'];
    $token = $_GET['token'];
	var_dump(DemoPushSingleDeviceNotification($token,$id));
}
...
function DemoPushAllDeviceMessage()
{
...
function DemoPushSingleDeviceNotification($token,$id)
{
	...
	$mess->setTitle("车辆维护警告");
	if((int)$id==1){
		$contentss='油量已不足20%，请及时到附近加油';
	}else if((int)$id==2){
		$contentss='里程已增加15000公里，请及时维护';
	}else if((int)$id==3){
		$contentss='车灯故障，请及时维护';
	}else if((int)$id==4){
		$contentss='变速器性能差，请及时维护';
	}else if((int)$id==5){
		$contentss='发动机性能差，请及时维护';
	}else if((int)$id==6){
		$contentss='您有新的评论了';
	}
