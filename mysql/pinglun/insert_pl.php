...
mysql_query($result);
if ($result) {
		$re=mysql_fetch_array(mysql_query("select user_phone from danmu where id='$pl_num'"));
		if($user_phone!=$re[0]){
			$sqlsss="select user_token from user_information where user_phone='$re[0]'";
			$rowssss = mysql_fetch_array(mysql_query($sqlsss));
			$url="http://115.159.26.120/androidapp_matrix/push/demo.php?id=6&token=".$rowssss[0];//推送用get请求完成
			access_url($url);
		}
        $response["success"] = 1;
